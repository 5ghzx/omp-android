...
private fun parseLink(input: String): Link {
    ...
    val uri = URI(text)
    val match = Regex("^/r/([A-Za-z0-9_-]{10,64})(?:\\.([A-Za-z0-9_-]+))?$").matchEntire(uri.path) ?: error("Invalid OMP collab link")
    val secretText = match.groupValues.getOrNull(2)?.takeIf { it.isNotEmpty() } ?: uri.rawFragment ?: error("Missing key")
...
private fun seal(key: ByteArray, json: String): ByteArray { val iv = ByteArray(12).also { SecureRandom().nextBytes(it) }; val cipher = Cipher.getInstance("AES/GCM/NoPadding"); cipher.init(Cipher.ENCRYPT_MODE, SecretKeySpec(key, "AES"), GCMParameterSpec(128, iv)); return iv + cipher.doFinal(json.toByteArray(Charsets.UTF_8)) }
private fun open(key: ByteArray, data: ByteArray): String { require(data.size > 12); val cipher = Cipher.getInstance("AES/GCM/NoPadding"); cipher.init(Cipher.DECRYPT_MODE, SecretKeySpec(key, "AES"), GCMParameterSpec(128, data.copyOfRange(0, 12))); return cipher.doFinal(data.copyOfRange(12, data.size)).toString(Charsets.UTF_8) }
private fun envelope(peer: Int, sealed: ByteArray): ByteArray = ByteBuffer.allocate(4 + sealed.size).putInt(peer).put(sealed).array()
private fun contentText(value: Any?): String = when (value) { is String -> value; is JSONArray -> buildString { for (i in 0 until value.length()) append(value.optJSONObject(i)?.optString("text").orEmpty()) }; else -> "" }
...
class MainActivity : ComponentActivity() {
    ...
    @Composable private fun OmpApp() {
        var linkText by remember { mutableStateOf("") }; var prompt by remember { mutableStateOf("") }; var status by remember { mutableStateOf("Disconnected") }; var lines by remember { mutableStateOf(listOf<Line>()) }; var socket by remember { mutableStateOf<WebSocket?>(null) }; var parsed by remember { mutableStateOf<Link?>(null) }; val scope = rememberCoroutineScope()
        ...
        fun connect() { try { val link = parseLink(linkText); parsed = link; lines = emptyList(); status = "Connecting…"; socket?.close(1000, "reconnect"); socket = OkHttpClient().newWebSocket(Request.Builder().url(link.wsUrl).build(), object : WebSocketListener() {
            override fun onOpen(webSocket: WebSocket, response: Response) { status = "Connected"; val hello = JSONObject().put("t", "hello").put("proto", PROTO).put("name", "OMP Android"); link.writeToken?.let { hello.put("writeToken", Base64.getUrlEncoder().withoutPadding().encodeToString(it)) }; webSocket.send(envelope(0, seal(link.key, hello.toString())).toByteString()) }
